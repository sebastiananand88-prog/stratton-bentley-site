...
export interface FaqCategory {
  category: string;
  /** Matches the page this category's content is most relevant to, for pulling a subset onto that page. */
  slug: string;
  items: FaqItem[];
}
...
export const FAQ_CATEGORIES: FaqCategory[] = [
  ...
  },
  {
    ...
    slug: "visual-stress-assessments",
    items: [
      {
...
/** Combine items from one or more categories -- e.g. for a page that covers two topics (varifocals + Essilor). */
export function getFaqItems(...slugs: string[]): FaqItem[] {
  return slugs.flatMap((slug) => getFaqCategory(slug)?.items ?? []);
}
